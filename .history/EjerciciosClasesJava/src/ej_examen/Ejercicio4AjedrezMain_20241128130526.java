    ...
    void mostrarSituacionPiezasTablero() {
        ...
        for (int i = 0; i < piezasEnTablero.length; i++) {
			...
			for (int j = 0; j < piezasEnTablero[i].length; j++) {
				...
				if (miPieza != null) {
					System.out.println( miPieza.tipoPieza.toString() + miPieza.color.toString() + " en posición " + i + "," + j );					
					cantidadPiezasEnTablero++;
				}
			}
		}
		//Mostrar cantidad de piezas
		System.out.println("Total de piezas en el tablero: " + cantidadPiezasEnTablero);
    }
}
